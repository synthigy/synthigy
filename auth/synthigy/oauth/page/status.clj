       ...
       [:div.status-card
        ...
        [:div.message
         (cond
           ...
           (and user client)
           ;; `*clients*` is keyed by the id seam — a UUID object in euuid mode,
           ;; an xid string in xid mode. `id/coerce-arg` matches both (and is
           ;; tolerant: it never throws on a non-UUID param like a raw
           ;; `UUID/fromString` would on an xid).
           (let [c (get @core/*clients* (id/coerce-arg client))]
