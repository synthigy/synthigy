          ...
          (fn [current-tokens]
            (reduce-kv
             (fn [tokens token-key data]
               (log/debug {:id ::adding-session-token
                           :data {:action :stored
...
(comment
  ...
  (java.util.Date. (* 1000 (:exp (iam/unsign-data (sign-token session :refresh_token data)))))
  (def token (first (keys (get @*tokens* :refresh_token))))
  (def token (first (keys (get @*tokens* :access_token))))
  (time (java.util.Date. (* 1000 (:exp (iam/unsign-data token)))))
...
;; =============================================================================
;; Identifier scopes — emit XIDs (the canonical Synthigy id)
;;
;; Synthigy is xid-native. These scopes RESOLVE the user / roles / groups
;; through their entities and emit the canonical XID (never a euuid). They are
;; registered under the `*:xid` names plus the legacy `*:uuid` aliases: the old
;; names predate the euuid→xid convergence and are kept so existing clients
;; (and frontend scope strings like "roles:uuid groups:uuid") keep working —
;; both forms now return XIDs.
;; =============================================================================

(defn- scope-sub-xid
  "Emit the resource owner's XID under `claim-key`."
  [session tokens claim-key]
  (let [resource-owner (core/get-session-resource-owner session)]
    (assoc-in tokens [:access_token claim-key] (id/extract resource-owner))))
...
(defn- scope-roles-xid
  ...
  (let [{:keys [roles]} (core/get-session-resource-owner session)
        role-xids (->> (dataset/search-entity :iam/user-role
                                              ...
                                              {(id/key) nil})
                       (mapv (id/key)))]
    (assoc-in tokens [:access_token :roles] role-xids)))
...
(defn- scope-groups-xid
  ...
  (let [{:keys [groups]} (core/get-session-resource-owner session)
        group-xids (->> (dataset/search-entity :iam/user-group
                                               ...
                                               {(id/key) nil})
                        (mapv (id/key)))]
    (assoc-in tokens [:access_token :groups] group-xids)))

(defmethod process-scope "sub:xid"     [session tokens _] (scope-sub-xid session tokens "sub:xid"))
(defmethod process-scope "sub:uuid"    [session tokens _] (scope-sub-xid session tokens "sub:uuid"))   ; legacy alias → xid
(defmethod process-scope "roles:xid"   [session tokens _] (scope-roles-xid session tokens))
(defmethod process-scope "roles:uuid"  [session tokens _] (scope-roles-xid session tokens))            ; legacy alias → xid
(defmethod process-scope "groups:xid"  [session tokens _] (scope-groups-xid session tokens))
(defmethod process-scope "groups:uuid" [session tokens _] (scope-groups-xid session tokens))           ; legacy alias → xid
