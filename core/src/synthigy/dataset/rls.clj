(ns synthigy.dataset.rls
  "Row-Level Security (RLS) Guards implementation.
   ...
   Uses pre-compiled schema from model->schema which resolves xid/UUID to
...
(defn should-apply-guards?
  "Check if RLS guards should be applied for current request.
   ...
   - No principal bound (system/internal operations)
   - Principal is superuser

   Note: RLS is independent of RBAC.
...
(defn- get-session-values
  "Get the session values to match against based on match type.
   Returns vector of _eid values projected from the current principal.

   `:role` match is DEPRECATED. Role-based row visibility should be
   modeled as a group with role-equivalent membership. Roles are RBAC
   identifiers (which entities/relations the principal can touch),
   groups are RLS identifiers (which rows the principal sees). Mixing
   the two collapses the unidirectional identifier discipline.

   Returning nil for `:role` causes the calling guard to deny all rows
   (`1=0`) — fail-closed by design."
  [match]
  (case match
    :user  (when-let [uid (access/principal-eid)] [uid])
    :group (when-let [gs (seq (access/group-eids))] (vec gs))
    :role  (do
             (log/warn {:id ::role-match-deprecated
                        :data {:action :denied :subject :rls-role-match}}
                       "RLS :role match is deprecated; model as a group instead. Denying.")
             nil)
    nil))
