    ...
    Returns:
      User context map with keys:
        {:_eid      Integer     ; Database entity ID
         ...
         :avatar    Map         ; Avatar (json attribute, not currently selected here)
...
(defn get-user-details
  "Fetch and materialize the principal map for a user. :roles/:groups are returned as maps keyed by id/key with full records (id/key, _eid, name) as values. :password is stripped."
  [args]
        ...
        {:_eid nil
         ...
         :person_info [{:args {:_join :left}
                        ...
                        {:name nil
                         ...
                         :address nil}}]
         ;; LEFT joins: a user with no person_info/groups/roles must still
         ;; be returned — relation pulls here are projections, not filters.
         ;; INNER (default) would drop the parent via inner-exists.
         :groups [{:args {:_join :left} :selections {(id/key) nil :_eid nil :name nil}}]
         :roles  [{:args {:_join :left} :selections {(id/key) nil :_eid nil :name nil}}]})
      (dissoc :password)
      (update :roles  materialize)
      (update :groups materialize))))
...
(defn ->CachedUserContextProvider
  "Create a cached user context provider with automatic cache invalidation.
   ...
   Uses lexical closure to maintain the cache. `start!` registers a
   `delta/subscribe!` matching User entity changes; the handler runs in
   delta's per-subscriber go-loop and invalidates the relevant cache
   entry. `stop!` tears the subscription down."
  []
  (let [cache (atom {})]
    (reify UserContextProvider
      ...
      (start! [this]
        ;; Resolve User entity once — its xid is stable across deploys.
        ;; sql-query/resolve-entity throws if User isn't in the model;
        ;; that's a deployment problem, not something to swallow here.
        (let [user-xid (sql-query/resolve-entity "User")]
          (delta/subscribe!
      ...
      (stop! [_]
        (delta/unsubscribe! ::user-cache-invalidator)
...
(defmacro with-user-context
  "Bind access/*principal* to the materialized user map for `identifier` and execute body.
   Throws IllegalArgumentException if user not found, IllegalStateException if no provider configured."
  [identifier & body]
