...
(lifecycle/register-module!
 ...
 {:depends-on [:synthigy/dataset]
  ...
  :start (fn []
           ;; Initialise the provider only. Keypair-existence is checked by
           ;; the storage-aware layer (oauth.persistence/open-store) after it
           ;; has loaded any persisted keypairs into the provider — checking
           ;; here would always see an empty provider and rotate needlessly,
           ;; since persistence runs later in the lifecycle DAG.
