...
;; Delegate to unified implementation in sql/query.clj
(def store-entity-records sql-query/store-entity-records)
...
(defn search-entity-roots
  ...
  ([connection schema]
   ...
   (let [focused-schema (focus-order schema)
         ...
         ;; SQLite doesn't support DISTINCT ON - check for attribute-based distinct
         distinct-attrs (get-in schema [:args :_distinct :attributes])
         ;; Use plain DISTINCT only when no specific attributes are requested
         distinct-on (when (and (not distinct-attrs)
                                (or (get-in schema [:args :_distinct])
                                    (and
                                      (contains? (:args focused-schema) :_limit)
                                      (not (contains? (:args focused-schema) :_order_by)))))
                       "DISTINCT")
         ;; For _distinct with attributes, use GROUP BY instead
           ...
           (let [roots (search-entity-roots connection schema)]
             (if (some? roots)
               (letfn [(construct-statement
                         ...
                         ;; SQLite: Use IN clause instead of =any(?)
                 ...
                 (let [db (pull-cursors connection schema roots)
                   ...
                   (doseq [query delete-statements]
                     ...
                     (sql/execute! connection query *return-type*))
        ...
        (let [targeting? (targeting-schema? schema)
          ...
          (cond
            ...
            (let [sql (format
                        ...
                        on' table)
                  tree (sql/execute! connection [sql] *return-type*)
                ...
                (pull-roots
                  ...
                  {(keyword as)
                   (distinct
                     (map
                       :_eid
                       (sql/execute! connection sql-final *return-type*)))})))
...
(defn aggregate-entity-tree
  ...
  (let [{:keys [entity/table entity/as]
         :as schema} (selection->schema
                       ...
                       args)
        on' (name on)
        ;; Get rows inside with-open, then call aggregate-entity outside
        ;; to avoid nested connection acquisition (SQLite pool = 1)
        rows (with-open [connection (jdbc/get-connection (:datasource *db*))]
               (when-let [found-roots (search-entity-roots
                                        connection
                                        (update schema :args dissoc :_distinct :_limit :_offset))]
                 (when-let [roots (get found-roots (keyword as))]
                   (let [placeholders (str/join "," (repeat (count roots) "?"))
                         sql (format
                               "with recursive tree(_eid,link,depth,path,cycle) as (
                               select
                               g._eid, g.%s, 1, ',' || g._eid || ',', 0
                               from %s g where g._eid IN (%s)
                               union all
                               select g._eid, g.%s, o.depth + 1, o.path || g._eid || ',',
                                      CASE WHEN INSTR(o.path, ',' || g._eid || ',') > 0 THEN 1 ELSE 0 END
                               from %s g, tree o
                               where g._eid=o.link and not cycle
                               ) select * from tree group by _eid"
                               on' table placeholders on' table)]
                     (log/tracef
                       "[%s] Get entity tree roots\n%s"
                       entity-id sql)
                     (when (not-empty roots)
                       (not-empty
                         (map
                           ...
                           (sql/execute!
                             ...
                             (into [sql] roots)
                             *return-type*))))))))]
    ;; Call aggregate-entity outside with-open to release connection first
    (if rows
         ...
         (when (and (not-empty args) table)
           (let [[statements data] (reduce-kv
                 ...
                 _ (log/tracef
                     "[%s] Deleting entity\n%s"
                     entity-id sql)]
             (sql/execute! connection sql *return-type*)
             (async/put! core/*delta-client* {:element entity-id
                                              :delta {:type :delete
                                                      :data args}})
             ; (async/put!
